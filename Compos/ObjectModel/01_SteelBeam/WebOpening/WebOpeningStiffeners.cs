...
namespace ComposAPI.SteelBeam
{
  ...
  public class WebOpeningStiffeners
  {
    ...
    {
      string start = (this.DistanceFrom.Value == 0) ? "" : "d:" + this.DistanceFrom.ToUnit(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0").Replace(" ", string.Empty);
      string top = (this.TopStiffenerWidth.Value == 0) ? "" : "Top:" + this.TopStiffenerWidth.As(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0").Replace(" ", string.Empty)
          + "x" + this.TopStiffenerThickness.ToUnit(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0").Replace(" ", string.Empty);
      string bottom = (this.BottomStiffenerWidth.Value == 0) ? "" : "Bottom:" + this.BottomStiffenerWidth.As(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0").Replace(" ", string.Empty)
          + "x" + this.BottomStiffenerThickness.ToUnit(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0").Replace(" ", string.Empty);
