...
namespace ComposAPI.Studs
{
  ...
  public class StudSpecification
  {
    ...
    {
      string noStudStart = (this.NoStudZoneStart.Value == 0) ? "" : "NSZS:" + this.NoStudZoneStart.ToUnit(Helpers.Units.FileUnits.LengthUnitGeometry).ToString("f0").Replace(" ", string.Empty);
      string noStudEnd = (this.NoStudZoneEnd.Value == 0) ? "" : "NSZE:" + this.NoStudZoneEnd.ToUnit(Helpers.Units.FileUnits.LengthUnitGeometry).ToString("f0").Replace(" ", string.Empty);
      string rebarPos = (this.ReinforcementPosition.Value == 0) ? "" : "RbP:" + this.ReinforcementPosition.ToUnit(Helpers.Units.FileUnits.LengthUnitGeometry).ToString("f0").Replace(" ", string.Empty);
