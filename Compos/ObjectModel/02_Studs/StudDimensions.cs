...
namespace ComposAPI.Studs
{
  ...
  public class StudDimensions
  {
    ...
    {
      string dia = Diameter.As(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0");
      string h = Height.ToUnit(Helpers.Units.FileUnits.LengthUnitSection).ToString("f0");
      string f = (Fu.Value == 0) ? CharacterStrength.ToUnit(Helpers.Units.FileUnits.ForceUnit).ToString("f0") : Fu.ToUnit(Helpers.Units.FileUnits.StressUnit).ToString("f0");
