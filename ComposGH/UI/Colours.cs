...
namespace ComposGH.UI
{
  ...
  public class Colour
  {
    public static bool isSelected(Color argsColor)
    {
      if (argsColor.R == defaultCol.R && argsColor.G == defaultCol.G && argsColor.B == defaultCol.B) // not selected)
        return false;
      else
        return true;
    }
    internal static Color defaultCol = Grasshopper.Instances.Settings.GetValue("DefaultPreviewColour", Color.White);
