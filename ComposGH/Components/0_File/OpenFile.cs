...
namespace ComposGH.Components
{
  ...
  public class OpenModel : GH_Component, IGH_VariableParameterComponent
  {
    ...
    {
      var fdi = new Rhino.UI.OpenFileDialog { Filter = "Compos Files(*.cob)|*.cob|All files (*.*)|*.*" }; //"GSA Files(*.gwa; *.gwb)|*.gwa;*.gwb|All files (*.*)|*.*"
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      if (DA.GetData(0, ref gh_typ))
      {
        if (gh_typ.Value is GH_String)
        {
          ...
          ComposFile composFile = new ComposFile().Open(fileName);
