...
namespace ComposGH.Components {
  public class CreateStudSpecEN : GH_OasysDropDownComponent {
    ...
    public CreateStudSpecEN() : base("StandardEN" + StudSpecificationGoo.Name.Replace(" ", string.Empty), "StudSpecsEN", "Look up a Standard EN " + StudSpecificationGoo.Description + " for a " + StudGoo.Description,
            Ribbon.CategoryName.Name(), Ribbon.SubCategoryName.Cat2()) { Hidden = true; } // sets the initial state of the component to hidden

    public override void SetSelected(int i, int j) {
      // change selected item
      _selectedItems[i] = _dropDownItems[i][j];
      if (_lengthUnit.ToString() == _selectedItems[i]) {
        return;
      }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      if (Params.Input[0].Sources.Count > 0) {
        noStudZoneStart = Input.LengthOrRatio(this, DA, 0, _lengthUnit, true);
      }
      IQuantity noStudZoneEnd = Length.Zero;
      if (Params.Input[1].Sources.Count > 0) {
        noStudZoneEnd = Input.LengthOrRatio(this, DA, 1, _lengthUnit, true);
      }

      // get rebar position
      var rebarPos = new Length(30, LengthUnit.Millimeter);
      if (Params.Input[2].Sources.Count > 0) {
        rebarPos = (Length)Input.UnitNumber(this, DA, 2, _lengthUnit, true);
      }
      bool welded = true;
      DA.GetData(3, ref welded);
      bool ncci = false;
      DA.GetData(4, ref ncci);
      var specEN = new StudSpecification(noStudZoneStart, noStudZoneEnd, rebarPos, welded, ncci);
