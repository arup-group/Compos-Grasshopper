...
namespace ComposGH.Components {
  public class CreateStandardStudDimensionsEN : GH_OasysDropDownComponent {
    ...
    private readonly List<string> _standardSizes = new List<string>(new string[] {
       "Custom",
      "Ø13/65mm",
      "Ø16/75mm",
      "Ø19/75mm",
    ...
    private StandardStudGrade _stdGrd = StandardStudGrade.SD1_EN13918;

    private StandardStudSize _stdSize = StandardStudSize.D19mmH100mm;

    private PressureUnit _stressUnit = DefaultUnits.MaterialStrengthUnit;

    public CreateStandardStudDimensionsEN() : base("StandardEN" + StudDimensionsGoo.Name.Replace(" ", string.Empty), "StudDimsEN", "Look up a Standard " + StudDimensionsGoo.Description + " for a " + StudGoo.Description, Ribbon.CategoryName.Name(), Ribbon.SubCategoryName.Cat2()) {
    ...
    public override void SetSelected(int i, int j) {
      ...
      {
        if (_selectedItems[0] != _standardSizes[0]) {
          string sz = _selectedItems[i].Replace("Ø", "D").Replace("/", "mmH");
          _stdSize = (StandardStudSize)Enum.Parse(typeof(StandardStudSize), sz);
      ...
      if (i == 1) {
        ...
        _stdGrd = (StandardStudGrade)Enum.Parse(typeof(StandardStudGrade), _selectedItems[i]);
      }
      if (i == 2) {
        // change is made to grade
    ...
    public override void VariableParameterMaintenance() {
      ...
      if (_selectedItems[0] == _standardSizes[0]) // custom size
      {
      ...
      if (_selectedItems[0] != _standardSizes[0]) // standard size
      {
    ...
    protected override void InitialiseDropdowns() {
      ...
      // spacing
      _dropDownItems.Add(_standardSizes);
      _selectedItems.Add(_stdSize.ToString().Replace("D", "Ø").Replace("mmH", "/"));

      // grade
      _dropDownItems.Add(Enum.GetValues(typeof(StandardStudGrade)).Cast<StandardStudGrade>().Select(x => x.ToString()).ToList());
      _selectedItems.Add(_stdGrd.ToString());
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      if (_selectedItems[0] == _standardSizes[0]) // custom size
      {
        var dia = (Length)Input.UnitNumber(this, DA, 0, _lengthUnit);
        var h = (Length)Input.UnitNumber(this, DA, 1, _lengthUnit);

        if (Params.Input[2].Sources.Count > 0) {
          _selectedItems[1] = "Custom";
          var strengthS = (Pressure)Input.UnitNumber(this, DA, 2, _stressUnit);
          Output.SetItem(this, DA, 0, new StudDimensionsGoo(new StudDimensions(dia, h, strengthS, StudSpecType.EC4)));
        }
        else {
          Output.SetItem(this, DA, 0, new StudDimensionsGoo(new StudDimensions(dia, h, _stdGrd)));
        }
      }
      else {
    ...
    protected override void UpdateUIFromSelectedItems() {
      if (_selectedItems[0] != _standardSizes[0]) {
        ...
        _stdSize = (StandardStudSize)Enum.Parse(typeof(StandardStudSize), sz);
        _stdGrd = (StandardStudGrade)Enum.Parse(typeof(StandardStudGrade), _selectedItems[1]);
      }
      else {
        ModeChangeClicked();
        _lengthUnit = (LengthUnit)UnitsHelper.Parse(typeof(LengthUnit), _selectedItems[3]);
      }
