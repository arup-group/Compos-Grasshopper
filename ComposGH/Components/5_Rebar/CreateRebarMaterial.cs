    ...
    public class RebarMat : GH_Component, IGH_VariableParameterComponent
    {
        ...
        public override void CreateAttributes()
        {
            if (first)
            {
                ...
                //material type
                dropdownitems.Add(Enum.GetValues(typeof(RebarMaterial.RebarMatType)).Cast<RebarMaterial.RebarMatType>().Select(x => x.ToString()).ToList());
        ...
        public void SetSelected(int i, int j)
        {
            ...
            if (i == 0) // change is made to type 
            {
                ...
                type = (RebarMaterial.RebarMatType)Enum.Parse(typeof(RebarMaterial.RebarMatType), selecteditems[i]);

                if (type == RebarMaterial.RebarMatType.Standard)
                {
                    //mat = (Rebar.StandardGrade)Enum.Parse(typeof(Rebar.StandardGrade), selecteditems[i]);
                    dropdownitems[1] = Enum.GetValues(typeof(RebarMaterial.StandardGrade)).Cast<RebarMaterial.StandardGrade>().Select(x => x.ToString()).ToList();
        ...
        {
            type = (RebarMaterial.RebarMatType)Enum.Parse(typeof(RebarMaterial.RebarMatType), selecteditems[0]);
            if (type == RebarMaterial.RebarMatType.Standard)
            {
                mat = (RebarMaterial.StandardGrade)Enum.Parse(typeof(RebarMaterial.StandardGrade), selecteditems[1]);
        ...
        private RebarMaterial.RebarMatType type = RebarMaterial.RebarMatType.Standard;
