...
namespace ComposGH.Components
{
    ...
    public class StandardDeck : GH_Component, IGH_VariableParameterComponent
    {
        ...
        public override Guid ComponentGuid => new Guid("06624453-6B39-4005-B04E-F4C3C1F54D6D");
        public StandardDeck()
        ...
        public void SetSelected(int i, int j)
        {
            ...
            if (selecteditems[0] == "Catalogue")
            {
                ...
                if (_mode != FoldMode.Catalogue | updateCat)
                {
                    ...
                    typedata = SqlReader.GetTypesDataFromSQLite(catalogueIndex, Path.Combine(AddReferencePriority.InstallPath, "decking.db3"), inclSS);
                    typeNames = typedata.Item1;
                    typeNumbers = typedata.Item2;

                    // update section list to all types
                    sectionList = SqlReader.GetSectionsDataFromSQLite(typeNumbers, Path.Combine(AddReferencePriority.InstallPath, "decking.db3"), inclSS);
                ...
                if (i == 1)
                {
                    ...
                    sectionList = SqlReader.GetSectionsDataFromSQLite(types, Path.Combine(AddReferencePriority.InstallPath, "decking.db3"), inclSS);
        ...
        // Catalogues
        readonly Tuple<List<string>, List<int>> cataloguedata = SqlReader.GetCataloguesDataFromSQLite(Path.Combine(AddReferencePriority.InstallPath, "decking.db3"));
        ...
        // Types
        Tuple<List<string>, List<int>> typedata = SqlReader.GetTypesDataFromSQLite(-1, Path.Combine(AddReferencePriority.InstallPath, "decking.db3"), false);
        ...
        List<string> sectionList = SqlReader.GetSectionsDataFromSQLite(new List<int> { -1 }, Path.Combine(AddReferencePriority.InstallPath, "decking.db3"), false);
